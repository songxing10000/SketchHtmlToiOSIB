    ...
    for (unsigned int i = 0; i < attrCount; i++) {
        switch (attrs[i].name[0]) {
            ...
            case 'V': { // Instance variable
                if (attrs[i].value) { _ivarName = [NSString stringWithUTF8String:attrs[i].value]; }
            } break;
            case 'R': {
...
+ (instancetype)classInfoWithClass:(Class)cls {
    ...
    YYClassInfo *info = CFDictionaryGetValue(class_isMetaClass(cls) ? metaCache : classCache,
